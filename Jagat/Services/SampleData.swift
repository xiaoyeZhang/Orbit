...
/// 演示用示例数据（仅在 Mock 后端中使用）。
enum SampleData {

    /// 默认城市中心（北京）。登录后地图先定位到这里，再跟随真实定位。
    static let cityCenter = Coordinate(latitude: 39.9042, longitude: 116.4074)
    ...
    static let messages: [String: [Message]] = [
        ...
        "c2": [
            Message(id: "m4", conversationId: "c2", senderId: "f2", kind: .location(Coordinate(latitude: 39.9151, longitude: 116.4210), name: "东四环辅路"), date: Date().addingTimeInterval(-1200), isRead: true),
        ],
        "c3": [
    ...
    static let places: [Place] = [
        Place(id: "p1", name: "公司", emoji: "🏢", coordinate: Coordinate(latitude: 39.9042, longitude: 116.4200), visitCount: 128, lastVisit: Date().addingTimeInterval(-3600)),
        Place(id: "p2", name: "家", emoji: "🏠", coordinate: Coordinate(latitude: 39.9300, longitude: 116.3800), visitCount: 240, lastVisit: Date().addingTimeInterval(-28800)),
        Place(id: "p3", name: "常去的咖啡馆", emoji: "☕️", coordinate: Coordinate(latitude: 39.9100, longitude: 116.4100), visitCount: 36, lastVisit: Date().addingTimeInterval(-86400)),
    ]

    // MARK: - 随机生成（用于"添加好友"演示）
    private static let names = ["Leo", "Nina", "大头", "Sky", "糖糖", "阿杰", "Yuki", "胖虎"]
    static func randomName() -> String { names.randomElement() ?? "新朋友" }

    static func randomAvatar() -> AvatarConfig {
        AvatarConfig(
